...
public class QBTwitterLDA {
	...
	// topic - tweet \& retweet
	private int[][] n_fz;// n_fz[0][z]: number of original tweets that are
							// associated with topic z
	private int[] sum_nfz;// sum_nfz[z] = n_fz[0]+n_fz[1]
	...
	private int[][] final_n_fz;
	private int[] final_sum_nfz;

	private double[] put;// probability that a tweet posted by u is an original
							// tweet
	...
	public void readData() {
		...
		try {
			...
			for (File retweetFile : retweetFolder.listFiles()) {
				// index of the user
				String userId = FilenameUtils.removeExtension(
						retweetFile.getName()).replace(".txt", "");
				u = userId2Index.get(userId);
				// read the retweet
				int nRetweet = 0;
				br = new BufferedReader(new FileReader(
						retweetFile.getAbsolutePath()));
				while (br.readLine() != null) {
					nRetweet++;
				}
				br.close();

				users[u].retweets = new Tweet[nRetweet];
				...
				while ((line = br.readLine()) != null) {
					j++;
					users[u].retweets[j] = new Tweet();
					String[] tokens = line.split(" ");
					users[u].retweets[j].tweetID = tokens[0];
					users[u].retweets[j].batch = Integer.parseInt(tokens[1]);
					users[u].retweets[j].words = new int[tokens.length - 2];
					for (int i = 0; i < tokens.length - 2; i++)
						users[u].retweets[j].words[i] = Integer
								.parseInt(tokens[i + 2]);
				}
				br.close();
			}
	...
	private void declareFinalCounts() {
		...
		final_n_fz = new int[2][nTopics];
		final_sum_nfz = new int[nTopics];
		for (int z = 0; z < nTopics; z++) {
			final_n_fz[0][z] = 0;
			final_n_fz[1][z] = 0;
			final_sum_nfz[z] = 0;
	...
	private void initilize() {
		...
		for (int u = 0; u < users.length; u++) {
			...
			// retweet
			if (users[u].retweets == null)
				continue;
			for (int t = 0; t < users[u].retweets.length; t++) {
				if (users[u].retweets[t].batch == testBatch)
					continue;
				users[u].retweets[t].topic = rand.nextInt(nTopics);
				int nWords = users[u].retweets[t].words.length;
				users[u].retweets[t].coins = new int[nWords];
				for (int i = 0; i < users[u].retweets[t].coins.length; i++)
					users[u].retweets[t].coins[i] = rand.nextInt(2);
			}
		}
		...
		n_fz = new int[2][nTopics];
		sum_nfz = new int[nTopics];
		for (int z = 0; z < nTopics; z++) {
			n_fz[0][z] = 0;
			n_fz[1][z] = 0;
			sum_nfz[z] = 0;
		...
		for (int u = 0; u < users.length; u++) {
			...
			for (int t = 0; t < users[u].retweets.length; t++) {
				if (users[u].retweets[t].batch == testBatch)
					continue;
				int z = users[u].retweets[t].topic;
				// user-topic and community-topic
				n_zu[z][u]++;
				sum_nzu[u]++;
				for (int i = 0; i < users[u].retweets[t].words.length; i++) {
					int w = users[u].retweets[t].words[i];
					int c = users[u].retweets[t].coins[i];
					// coin count
					n_c[c]++;
					sum_nc++;
					if (c == 0) {
	...
	private void sampleTweetTopic(int u, int t) {
		...
		for (int z = 0; z < nTopics; z++) {
			...
			p[z] = p[z] * (n_fz[0][z] + eta[0]) / (sum_nfz[z] + sum_eta);
			// cumulative
			p[z] = sump + p[z];
			sump = p[z];
		}
	...
	private void sampleRetweetTopic(int u, int t) {
		// sample the topic for tweet number t of user number u
		// get current topic
		int currz = users[u].retweets[t].topic;
		// sampling based on user interest
		n_zu[currz][u]--;
		sum_nzu[u]--;
		for (int i = 0; i < users[u].retweets[t].words.length; i++) {
			if (users[u].retweets[t].coins[i] == 0)
				continue;// do not consider background words
			int w = users[u].retweets[t].words[i];
			n_wz[w][currz]--;
			sum_nwz[currz]--;
		...
		for (int z = 0; z < nTopics; z++) {
			...
			p[z] = p[z] * (n_fz[1][z] + eta[1]) / (sum_nfz[z] + sum_eta);
			// cumulative
			p[z] = sump + p[z];
			sump = p[z];
		}
		...
		System.out.println("bug in sampleRetweetTopic");
		for (int z = 0; z < nTopics; z++) {
			System.out.print(p[z] + " ");
		}
		System.exit(-1);
	...
	private void sampleTweetWordCoin(int u, int t, int i) {
	...
	private void sampleRetweetWordCoin(int u, int t, int i) {
		// sample the coin for the word number i of the retweet number t of user
		// number u
		// get current coin
		int currc = users[u].retweets[t].coins[i];
		// get current word
		int w = users[u].retweets[t].words[i];
		// get current topic
		int z = users[u].retweets[t].topic;
	...
	private void updateFinalCounts() {
		...
		for (int z = 0; z < nTopics; z++) {
			final_n_fz[0][z] += n_fz[0][z];
			final_n_fz[1][z] += n_fz[1][z];
			final_sum_nfz[z] += sum_nfz[z];
		}
	}
	...
	private void inferingModelParameters() {
		...
		// retweet bias
		retweetBias = new double[nTopics][2];
		for (int z = 0; z < nTopics; z++) {
			retweetBias[z][0] = (final_n_fz[0][z] + eta[0])
					/ (final_sum_nfz[z] + sum_eta);
			retweetBias[z][1] = (final_n_fz[1][z] + eta[1])
					/ (final_sum_nfz[z] + sum_eta);
		}

		put = new double[users.length];
		for (int u = 0; u < users.length; u++) {
			put[u] = 0;
			for (int z = 0; z < nTopics; z++) {
				put[u] += users[u].topicDistribution[z] * retweetBias[z][0];
			}
		}
	}
	...
	private void outputTweetTopicTopTweets(int k) {
		...
		try {
			...
			bw.write("background\n");
			if (backgroundTweetID != null) {
				topTweets = rankTool.getTopKbyWeight(backgroundTweetID,
						backgroundTweetPerplexity,
						Math.min(2 * k, backgroundTweetID.length));
	...
	private void outputRetweetBias() {
		try {
			String fileName = outputPath + "/retweetBias.csv";
			File file = new File(fileName);
			if (!file.exists()) {
				file.createNewFile();
			}
			...
			System.out.println("Error in writing out retweet bias to file!");
			e.printStackTrace();
			System.exit(0);
		}
	}
